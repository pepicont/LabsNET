...
namespace Lab04
{
    ...
    {
        //en orm usé la que se llama desktop y acá la localdb
        protected readonly string connectionString = "server = serverisi; database=net;uid=net;pwd=net;"; 
        
        public override DataTable getTabla()
        {
            //es buena práctica usar using para manejar conexiones y comandos
            //quedaría using MySqlConnection ... { using MySqlCommand {...}}
            using (MySqlConnection conn = new MySqlConnection(this.connectionString))
            {
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM contactos", conn);
                conn.Open();
